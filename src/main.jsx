...
function defaultState() {
  return {
    username: "", usernameSet: false, activePage: "trading",
    selectedYear: new Date().getFullYear(), selectedMonth: new Date().getMonth(),
    tradeJournal: {}, paperBalance: 0, paperStartBalance: 0,
    realBalance: 0, realStartBalance: 0, accountGoal: 10000000,
    news: [], tasks: { morning: [], afternoon: [], evening: [] },
    ...
    weight: { current: 0, goal: 0, start: 0 }, balanceHistory: [],
  };
}
...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  input, textarea, select { background: ${G.card}; border: 1px solid ${G.border}; color: ${G.text}; border-radius: 6px; padding: 8px 12px; font-family: inherit; font-size: 13px; outline: none; transition: border .2s; }
...
const Card = ({ children, style, className="" }) => (
  <div className={`fade-in ${className}`} style={{ background: G.card, border: `1px solid ${G.border}`, borderRadius: 12, padding: 16, ...style }}>{children}</div>
);
const Btn = ({ children, onClick, color=G.accent, small, style, disabled }) => (
  <button onClick={onClick} disabled={disabled} style={{ background: "transparent", border: `1px solid ${color}`, color, borderRadius: 6, padding: small?"5px 10px":"8px 16px", fontSize: small?11:13, fontWeight: 600, letterSpacing: 1, opacity: disabled ? .4 : 1, ...style }}
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:"100vh", gap:24 }}>
      ...
      <div style={{ display:"flex", gap:8 }}>
        <input value={val} onChange={e=>setVal(e.target.value)} placeholder="your_username" style={{ width:220 }} onKeyDown={e=>e.key==="Enter"&&val.trim()&&onSet(val.trim())} />
        <Btn onClick={()=>val.trim()&&onSet(val.trim())} color={G.green}>ENTER</Btn>
      </div>
      <div style={{ color:G.muted, fontSize:11, maxWidth:300, textAlign:"center" }}>Same username on any device = synced data ✓</div>
    </div>
  );
}
function Nav({ activePage, setPage, username }) {
  const pages = [{ id:"trading", label:"📈 Trading" }, { id:"growth", label:"🧠 Personal Growth" }, { id:"fitness", label:"💪 Fitness" }];
  return (
    <div style={{ position:"sticky", top:0, zIndex:100, background:G.bg+"ee", backdropFilter:"blur(12px)", borderBottom:`1px solid ${G.border}`, padding:"10px 16px", display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
      <div className="orb" style={{ color:G.accent, fontSize:14, letterSpacing:3, marginRight:8 }}>TRADER HQ</div>
      {pages.map(p=>(
        <button key={p.id} onClick={()=>setPage(p.id)} style={{ background: activePage===p.id ? G.accent+"22" : "transparent", border: `1px solid ${activePage===p.id ? G.accent : G.border}`, color: activePage===p.id ? G.accent : G.muted, borderRadius:6, padding:"6px 14px", fontSize:12, fontWeight:600, letterSpacing:.5 }}>{p.label}</button>
      ))}
      <div style={{ marginLeft:"auto", color:G.muted, fontSize:11, fontFamily:"JetBrains Mono" }}>@{username}</div>
    </div>
  );
}
function buildQuarterStats(journal, year, quarterMonths) {
...
async function fetchAISummary(stats, quarterLabel, year, realBalance, paperBalance, milestoneIdx) {
  ...
  const prompt = `You are a professional trading coach reviewing a trader's quarterly performance. Be specific, insightful, encouraging but honest.\n\nQUARTERLY PERFORMANCE — ${quarterLabel} ${year}\nTrading Days: ${stats.tradeDays}\nWinning Days: ${stats.wins}\nLosing Days: ${stats.losses}\nWin Rate: ${stats.winRate}%\nTotal P&L: $${stats.totalPnl.toFixed(2)}\nReal Account: $${realBalance.toFixed(2)}\nPaper Money: $${paperBalance.toFixed(2)}\nMilestone: $${prevMilestone} → $${nextMilestone}\n${stats.notes.length ? `Trade Notes:\n${stats.notes.map((n,i)=>`${i+1}. ${n}`).join("\n")}` : ""}\n\nWrite a quarterly review with:\n1. **Quarter Overview** - 2-3 sentence summary\n2. **What Went Well** - 2-3 strengths\n3. **Areas to Improve** - 2-3 coaching points\n4. **Milestone Update** - progress toward goals\n5. **Focus for Next Quarter** - 3 specific goals\n\nUnder 400 words. Be direct and motivating.`;
...
function QuarterlyAISummary({ state }) {
  ...
  const stats = buildQuarterStats(state.tradeJournal || {}, selectedYear, q.months);
    ...
    <div style={{ background: G.card, border: `1px solid ${q.color}44`, borderRadius: 12, padding: 16 }}>
      <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14, flexWrap:"wrap" }}>
        ...
        <div style={{ display:"flex", gap:6, marginLeft:"auto", flexWrap:"wrap" }}>
          <select value={selectedYear} onChange={e=>setSelectedYear(Number(e.target.value))} style={{ padding:"4px 8px", fontSize:12 }}>
            {[now.getFullYear()-1, now.getFullYear(), now.getFullYear()+1].map(y=>(<option key={y} value={y}>{y}</option>))}
          </select>
          {QUARTERS.map((qq,i)=>(
            <button key={i} onClick={()=>setSelectedQ(i)} style={{ background: selectedQ===i ? qq.color+"33":"transparent", border:`1px solid ${selectedQ===i?qq.color:G.border}`, color: selectedQ===i?qq.color:G.muted, borderRadius:6, padding:"4px 10px", fontSize:11, fontWeight:700, cursor:"pointer" }}>{qq.label}</button>
          ))}
        </div>
      </div>
      <div style={{ display:"flex", gap:10, marginBottom:14, flexWrap:"wrap" }}>
        {[["📅 Days",stats.tradeDays,G.text],["🟢 Wins",stats.wins,G.green],["🔴 Losses",stats.losses,G.red],["📊 Win Rate",`${stats.winRate}%`,q.color],["💰 P&L",`${stats.totalPnl>=0?"+":""}${fmtDollar(stats.totalPnl)}`,stats.totalPnl>=0?G.green:G.red]].map(([l,v,c])=>(
          <div key={l} style={{ background:G.card2, border:`1px solid ${G.border}`, borderRadius:8, padding:"6px 12px" }}>
      ...
      {existing ? (
        <div style={{ background:G.card2, border:`1px solid ${q.color}33`, borderRadius:10, padding:16, marginBottom:12 }}>
          <div style={{ fontSize:10, color:G.muted, marginBottom:10 }}>Generated: {existing.generated}</div>
          <div style={{ fontSize:13, lineHeight:1.75, color:G.text }}>{renderText(existing.text)}</div>
        </div>
      ) : (
        <div style={{ textAlign:"center", padding:"20px 0", color:G.muted, fontSize:13, marginBottom:12 }}>
          {stats.tradeDays === 0 ? `No trades logged for ${q.label} ${selectedYear} yet.` : `Click below to generate your ${q.label} ${selectedYear} AI review.`}
        </div>
      )}
      {error && <div style={{ color:G.red, fontSize:12, marginBottom:8, textAlign:"center" }}>{error}</div>}
      <div style={{ display:"flex", justifyContent:"center" }}>
        <button onClick={generate} disabled={loading||stats.tradeDays===0} style={{ background:"transparent", border:`1px solid ${q.color}`, color:q.color, borderRadius:8, padding:"10px 24px", fontSize:12, fontWeight:700, cursor:loading||stats.tradeDays===0?"not-allowed":"pointer", opacity:stats.tradeDays===0?0.4:1, display:"flex", alignItems:"center", gap:8 }}>
          {loading ? (<><span style={{ display:"inline-block", width:12, height:12, border:`2px solid ${q.color}`, borderTopColor:"transparent", borderRadius:"50%", animation:"spin 0.8s linear infinite" }} />Analyzing...</>) : existing ? `🔄 Regenerate ${q.label}` : `✨ Generate ${q.label} ${selectedYear} Review`}
        </button>
      </div>
    </div>
  );
...
function TradingPage({ state, setState }) {
  ...
  function saveDay() {
    ...
    setState(s=>({ ...s, tradeJournal:updated }));
    setCalModal(null);
  }
  function saveBalance() {
    setState(s=>({ ...s, paperBalance: balForm.paper!=""?parseFloat(balForm.paper):s.paperBalance, realBalance: balForm.real!=""?parseFloat(balForm.real):s.realBalance, paperStartBalance: s.paperStartBalance||parseFloat(balForm.paper)||0, realStartBalance: s.realStartBalance||parseFloat(balForm.real)||0 }));
  ...
  const dayColor = (d) => { if (!d) return "transparent"; const key=`${year}-${String(month+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`; const e=journal[key]; if (!e||e.result==="neutral") return G.card2; return e.result==="green" ? G.green+"33" : G.red+"33"; };
  const dayBorder = (d) => { if (!d) return "transparent"; const key=`${year}-${String(month+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`; const e=journal[key]; if (!e||e.result==="neutral") return G.border; return e.result==="green" ? G.green+"88" : G.red+"88"; };
  let monthPnl=0, wins=0, losses=0, neutral=0;
  for (let d=1;d<=daysInMonth;d++) { const key=`${year}-${String(month+1).padStart(2,"0")}-${String(d).padStart(2,"0")}`; const e=journal[key]; if(e){ monthPnl+=e.pnl||0; if(e.result==="green")wins++; else if(e.result==="red")losses++; else neutral++; } }
  ...
  return (
    <div style={{ padding:16, display:"flex", flexDirection:"column", gap:16 }}>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 320px", gap:16 }}>
        <Card style={{ padding:0, overflow:"hidden" }}>
          <div style={{ padding:"10px 14px", borderBottom:`1px solid ${G.border}`, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            ...
            <a href="https://finviz.com/map.ashx" target="_blank" rel="noreferrer" style={{ color:G.muted, fontSize:11, textDecoration:"none" }}>Open Finviz ↗</a>
          </div>
          <iframe src="https://finviz.com/publish/072014/map.ashx?t=sec&st=ytd" style={{ width:"100%", height:320, border:"none", display:"block" }} title="Finviz Map" />
          <div style={{ padding:"8px 14px", background:G.card2, borderTop:`1px solid ${G.border}`, fontSize:11, color:G.muted }}>⚠ If map doesn't load: <a href="https://finviz.com/map.ashx" target="_blank" rel="noreferrer" style={{ color:G.accent }}>click here</a></div>
        </Card>
          ...
          {showNewsForm && (
            <div style={{ display:"flex", flexDirection:"column", gap:6, padding:10, background:G.card2, borderRadius:8, border:`1px solid ${G.border}` }}>
              ...
              <select value={newsForm.type} onChange={e=>setNewsForm(f=>({...f,type:e.target.value}))}>
                <option value="news">📰 News</option><option value="earnings">💰 Earnings</option><option value="release">🚀 New Release</option><option value="macro">🌐 Macro</option>
              </select>
              <div style={{ display:"flex", gap:6 }}><Btn small color={G.gold} onClick={addNews}>Save</Btn><Btn small color={G.muted} onClick={()=>setShowNewsForm(false)}>Cancel</Btn></div>
            </div>
          )}
            ...
            {(state.news||[]).map(n=>(
              <div key={n.id} style={{ padding:"8px 10px", background:G.card2, borderRadius:7, border:`1px solid ${G.border}` }}>
                <div style={{ display:"flex", gap:6, alignItems:"center", marginBottom:3 }}>
                  <Tag c={n.type==="earnings"?G.gold:n.type==="release"?G.purple:n.type==="macro"?G.blue:G.accent} label={n.type==="earnings"?"EARN":n.type==="release"?"RELE":n.type==="macro"?"MACRO":"NEWS"} />
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
        {[{ key:"paper", label:"PAPER MONEY", bal:state.paperBalance||0, start:state.paperStartBalance||0, color:G.blue },{ key:"real", label:"REAL ACCOUNT", bal:state.realBalance||0, start:state.realStartBalance||0, color:G.green }].map(acc=>{
          const pnl = acc.bal - acc.start; const pct = acc.start ? ((pnl/acc.start)*100).toFixed(1) : 0;
          return (
            <Card key={acc.key} style={{ padding:14 }}>
              ...
              <div style={{ marginTop:4, fontSize:12 }}>
                <span style={{ color:pnl>=0?G.green:G.red, fontFamily:"JetBrains Mono" }}>{pnl>=0?"▲":"▼"} {fmtDollar(Math.abs(pnl))} ({pct}%)</span>
      ...
      {showBalForm && (
        <div style={{ position:"fixed", inset:0, background:"#000a", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center" }} onClick={()=>setShowBalForm(false)}>
          <Card onClick={e=>e.stopPropagation()} style={{ width:320, padding:20, display:"flex", flexDirection:"column", gap:10 }}>
            ...
            <div style={{ display:"flex", gap:8 }}><Btn color={G.green} onClick={saveBalance}>Save</Btn><Btn color={G.muted} onClick={()=>setShowBalForm(false)}>Cancel</Btn></div>
          </Card>
        </div>
      )}
      <Card style={{ padding:14 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:12, flexWrap:"wrap" }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            <Tag c={G.green} label={`✓ ${wins}`} /><Tag c={G.red} label={`✗ ${losses}`} /><Tag c={G.muted} label={`– ${neutral}`} /><Tag c={monthPnl>=0?G.green:G.red} label={fmtDollar(monthPnl)} />
          </div>
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:4 }}>
          {dayCells.map((d,i)=>{
            ...
            return (
              <div key={key} onClick={()=>openDay(key)} style={{ background:dayColor(d), border:`1px solid ${isToday?G.accent:dayBorder(d)}`, borderRadius:6, padding:"6px 4px", textAlign:"center", cursor:"pointer", minHeight:44, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", transition:"all .15s" }}
                onMouseEnter={e=>e.currentTarget.style.transform="scale(1.05)"}
                onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}>
                <div className="mono" style={{ fontSize:12, fontWeight:isToday?700:400, color:isToday?G.accent:G.text }}>{d}</div>
                {e && e.pnl!==0 && <div style={{ fontSize:9, color:e.result==="green"?G.green:G.red, fontFamily:"JetBrains Mono" }}>{e.pnl>0?"+":""}{e.pnl}</div>}
              </div>
            );
          })}
        </div>
          ...
          <Card onClick={e=>e.stopPropagation()} style={{ width:340, padding:20, display:"flex", flexDirection:"column", gap:12 }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              {[["green","🟢 PROFIT",G.green],["red","🔴 LOSS",G.red],["neutral","⚪ NO TRADE",G.muted]].map(([v,l,c])=>(
                <button key={v} onClick={()=>setModalForm(f=>({...f,result:v}))} style={{ flex:1, padding:"8px 4px", borderRadius:6, fontSize:11, fontWeight:700, background:modalForm.result===v?c+"33":"transparent", border:`1px solid ${modalForm.result===v?c:G.border}`, color:modalForm.result===v?c:G.muted }}>{l}</button>
              ))}
            </div>
            <input type="number" placeholder="P&L amount" value={modalForm.pnl} onChange={e=>setModalForm(f=>({...f,pnl:e.target.value}))} />
            <textarea placeholder="Trade notes..." rows={3} value={modalForm.note} onChange={e=>setModalForm(f=>({...f,note:e.target.value}))} style={{ resize:"vertical" }} />
            <div style={{ display:"flex", gap:8 }}><Btn color={G.green} onClick={saveDay}>Save</Btn><Btn color={G.muted} onClick={()=>setCalModal(null)}>Cancel</Btn></div>
          </Card>
        </div>
      )}
      ...
      <Card style={{ padding:14 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
          {MILESTONES.map((m,i)=>{
            const prev = i===0?0:MILESTONES[i-1]; const achieved = realBal >= m; const active = !achieved && realBal >= prev;
            const pct = active ? Math.min(100,((realBal-prev)/(m-prev))*100) : achieved?100:0;
            return (
              <div key={m} style={{ display:"flex", alignItems:"center", gap:10 }}>
                <div style={{ width:72, textAlign:"right", fontSize:11, fontFamily:"JetBrains Mono", color:achieved?G.gold:active?G.green:G.muted }}>{fmt(m)}</div>
                <div style={{ flex:1, background:G.border, borderRadius:99, height:7, overflow:"hidden" }}>
                  <div style={{ width:`${pct}%`, height:"100%", borderRadius:99, background:achieved?`linear-gradient(90deg,${G.gold},${G.gold}88)`:active?`linear-gradient(90deg,${G.green},${G.accent})`:G.border, transition:"width .5s" }} />
                </div>
                <div style={{ width:36, fontSize:10, fontFamily:"JetBrains Mono", color:achieved?G.gold:active?G.green:G.muted }}>{achieved?"✓":active?`${pct.toFixed(0)}%`:""}</div>
              </div>
            );
          })}
        </div>
...
function GrowthPage({ state, setState }) {
  ...
  function addTask(block) {
    ...
    setState(s=>{ const tasks = { ...s.tasks }; tasks[block] = [...(tasks[block]||[]), { id:Date.now(), text:newTask[block], done:false }]; return { ...s, tasks }; });
    setNewTask(f=>({...f,[block]:""}));
  }
  function toggleTask(block, id) { setState(s=>{ const tasks = { ...s.tasks }; tasks[block] = tasks[block].map(t=>t.id===id?{...t,done:!t.done}:t); return { ...s, tasks }; }); }
  function deleteTask(block, id) { setState(s=>{ const tasks = { ...s.tasks }; tasks[block] = tasks[block].filter(t=>t.id!==id); return { ...s, tasks }; }); }
    ...
    <div style={{ padding:16, display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:16 }}>
        {TIME_BLOCKS.map((block,bi)=>{
          const c = colors[bi]; const tasks = state.tasks?.[block.id]||[]; const done = tasks.filter(t=>t.done).length;
          return (
            <Card key={block.id} style={{ padding:14, border:`1px solid ${c}33` }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:12 }}>
                <span style={{ fontSize:24 }}>{block.icon}</span>
                <div><div className="orb" style={{ fontSize:11, color:c, letterSpacing:2 }}>{block.label}</div><div style={{ fontSize:12, color:G.muted }}>{block.time}</div></div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:5, marginBottom:10, maxHeight:200, overflowY:"auto" }}>
                {tasks.map(t=>(
                  <div key={t.id} style={{ display:"flex", alignItems:"center", gap:8, padding:"6px 8px", background:G.card2, borderRadius:6 }}>
                    <input type="checkbox" checked={t.done} onChange={()=>toggleTask(block.id,t.id)} style={{ width:14, height:14, accentColor:c }} />
              ...
              <div style={{ display:"flex", gap:6 }}>
                <input placeholder="Add task..." value={newTask[block.id]} onChange={e=>setNewTask(f=>({...f,[block.id]:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&addTask(block.id)} style={{ flex:1, fontSize:12, padding:"6px 10px" }} />
...
function FitnessPage({ state, setState }) {
  ...
  function toggleDayDone(d) { setState(s=>{ const w={...s.workouts}; w[d]={...(w[d]||{}), done:!(w[d]?.done)}; return {...s,workouts:w}; }); }
  function addExercise(d) {
    if (!exInput.trim()) return;
    setState(s=>{ const w={...s.workouts}; w[d]={...(w[d]||{}), exercises:[...(w[d]?.exercises||[]), { id:Date.now(), text:exInput, sets:"", reps:"" }]}; return {...s,workouts:w}; });
    setExInput("");
  }
  function removeExercise(d, id) { setState(s=>{ const w={...s.workouts}; w[d]={...(w[d]||{}), exercises:(w[d]?.exercises||[]).filter(e=>e.id!==id)}; return {...s,workouts:w}; }); }
  function updateExField(d, id, field, val) { setState(s=>{ const w={...s.workouts}; w[d]={...(w[d]||{}), exercises:(w[d]?.exercises||[]).map(e=>e.id===id?{...e,[field]:val}:e)}; return {...s,workouts:w}; }); }
  function saveWeight() { setState(s=>({ ...s, weight:{ current:parseFloat(weightForm.current)||s.weight?.current||0, start:parseFloat(weightForm.start)||s.weight?.start||0, goal:parseFloat(weightForm.goal)||s.weight?.goal||0 }})); setShowWeightForm(false); }
  const doneCount = Object.values(workouts).filter(w=>w?.done).length;
  const weightPct = (weight.start&&weight.goal&&weight.current) ? Math.min(100,Math.max(0,((weight.current-weight.start)/(weight.goal-weight.start))*100)) : 0;
    ...
    <div style={{ padding:16, display:"flex", flexDirection:"column", gap:16 }}>
      <Card style={{ padding:14 }}>
        ...
        <div style={{ display:"flex", gap:16, marginBottom:10 }}>
          {[["START",weight.start||"—",G.muted],["CURRENT",weight.current||"—",G.purple],["GOAL",weight.goal||"—",G.green]].map(([l,v,c])=>(
            <div key={l}><div style={{ fontSize:10, color:G.muted, marginBottom:2 }}>{l}</div><div className="mono" style={{ fontSize:20, color:c, fontWeight:700 }}>{v}{v!=="—"?" lbs":""}</div></div>
          ))}
          {weight.current&&weight.goal&&<div style={{ marginLeft:"auto" }}><div style={{ fontSize:10, color:G.muted, marginBottom:2 }}>REMAINING</div><div className="mono" style={{ fontSize:20, color:G.gold, fontWeight:700 }}>{Math.abs(weight.goal-weight.current).toFixed(1)} lbs</div></div>}
        </div>
        <div style={{ background:G.border, borderRadius:99, height:14, overflow:"hidden" }}>
          <div style={{ width:`${Math.min(100,adjustedPct)}%`, height:"100%", background:`linear-gradient(90deg,${G.purple},${G.accent})`, borderRadius:99, transition:"width .5s" }} />
        </div>
      </Card>
      <Card style={{ padding:14 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
        ...
        <div style={{ display:"flex", gap:8, overflowX:"auto", paddingBottom:4 }}>
          {WORKOUT_DAYS.map((day,d)=>{
            const w=workouts[d]||{};
            return (
              <button key={d} onClick={()=>setActiveDay(d)} style={{ flex:"0 0 auto", width:72, padding:"10px 6px", borderRadius:8, background:activeDay===d?(w.done?G.green+"33":G.card2):(w.done?G.green+"22":"transparent"), border:`1px solid ${activeDay===d?(w.done?G.green:G.accent):(w.done?G.green+"55":G.border)}`, color:activeDay===d?(w.done?G.green:G.accent):(w.done?G.green:G.muted), fontSize:11, fontWeight:700, cursor:"pointer", textAlign:"center" }}>
      ...
      <Card style={{ padding:14 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
          ...
          <Btn small color={(workouts[activeDay]?.done)?G.green:G.muted} onClick={()=>toggleDayDone(activeDay)}>{workouts[activeDay]?.done?"✓ DONE":"MARK DONE"}</Btn>
        </div>
        <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:10 }}>
          {(workouts[activeDay]?.exercises||[]).map(ex=>(
            <div key={ex.id} style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 10px", background:G.card2, borderRadius:7, border:`1px solid ${G.border}` }}>
              ...
              <input value={ex.sets} onChange={e=>updateExField(activeDay,ex.id,"sets",e.target.value)} placeholder="Sets" style={{ width:52, fontSize:11, padding:"4px 6px", textAlign:"center" }} />
              <input value={ex.reps} onChange={e=>updateExField(activeDay,ex.id,"reps",e.target.value)} placeholder="Reps" style={{ width:52, fontSize:11, padding:"4px 6px", textAlign:"center" }} />
          ...
          {!(workouts[activeDay]?.exercises?.length) && <div style={{ color:G.muted, fontSize:12, textAlign:"center", padding:12 }}>No exercises logged for this day</div>}
        </div>
        <div style={{ display:"flex", gap:8 }}>
          <input placeholder="Add exercise (e.g. Bench Press)" value={exInput} onChange={e=>setExInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addExercise(activeDay)} style={{ flex:1 }} />
          <Btn color={G.green} onClick={()=>addExercise(activeDay)}>+ ADD</Btn>
        </div>
        <textarea placeholder="Workout notes..." rows={2} value={workouts[activeDay]?.notes||""} onChange={e=>setState(s=>{ const w={...s.workouts}; w[activeDay]={...(w[activeDay]||{}),notes:e.target.value}; return {...s,workouts:w}; })} style={{ width:"100%", marginTop:10, resize:"vertical", fontSize:12 }} />
      </Card>
    </div>
  );
}
function App() {
  ...
  if (!state || !state.usernameSet) return (<><style>{css}</style><UsernameGate onSet={handleUsername} /></>);
  return (
    <>
      <style>{css}</style>
